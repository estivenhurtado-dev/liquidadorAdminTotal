...
import com.fenalcovalle.estandar.vista.dto.CorreoTemplateDTO;
import com.fenalcovalle.estandar.vista.dto.CorreoTemplateNotificacionDTO;
...
private String mensajeRespuestaPagare = "";
	
	private boolean botonFinalizar=false;

	

	public void inicarProceso() throws Exception {
...
//		try {
			...
			RequestBody body = RequestBody.create(mediaType, "{\r\n    \"idSolicitud\": " + idSolicitudBpm + "\r\n}");
			Request request = new Request.Builder().url(Parametros.URL_CREACION_PAGARE).method("POST", body)
					.addHeader("Authorization", "Bearer " + getToken()).addHeader("Content-Type", "application/json")
					.build();
			Response response = client.newCall(request).execute();
			Gson gson = new Gson();
			com.fenalcovalle.estandar.vista.dto.RespuestaPagareCustomDTO respuestaPagareDto = gson
					.fromJson(response.body().string(), com.fenalcovalle.estandar.vista.dto.RespuestaPagareCustomDTO.class);

			mensajeRespuestaPagare = respuestaPagareDto.getMessage();
			
//			System.out.println(respuestaPagareDto.getData().getDesc());
			
			botonFinalizar=true;
			
			
			 if (respuestaPagareDto.getType().equalsIgnoreCase("EXCEPTION_KEY_VALUE")) {
				 mensajeRespuestaPagare = respuestaPagareDto.getData().getDesc();
			 }

//		} catch (Exception e) {
//			mensajeRespuestaPagare = "Ocurrio un al generar el pagare revise el servicio fenalco_integ_pagares";
//		}
	...
	public String getToken() {
		...
		try {
			...
			byte[] message = (Parametros.USUARIO_SERVICIO_AUTH + ":" + Parametros.CONTRASENA_SERVICIO_AUTH)
					.getBytes("UTF-8");
			...
			RequestBody body = RequestBody.create(mediaType, "username=" + Parametros.USUARIO_SERVICIO + "&password="
					+ Parametros.CONTRASENA_SERVICIO + "&grant_type=" + Parametros.GRANT_TYPE + "");
			Request request = new Request.Builder().url(Parametros.URL_SERVICIO_AUTH).method("POST", body)
					.addHeader("Content-Type", "application/x-www-form-urlencoded")
					.addHeader("Authorization", "Basic " + encoded).build();
			Response response = client.newCall(request).execute();

			//
			Gson gson = new Gson();
			com.fenalcovalle.estandar.vista.dto.Token token = gson.fromJson(response.body().string(),
					com.fenalcovalle.estandar.vista.dto.Token.class);
			//
			//
			return token.getAccess_token();

		} catch (Exception e) {
			mensajeRespuestaPagare = "Ocurrio un al generar el token revise el servicio fenalco_integ_pagares";
			return "Ocurrio un al generar el token revise el servicio fenalco_integ_pagares";
		}
	}
	
	
	public void logLiquidacion()  {
		...
		try {
			...
			CorreoTemplateNotificacionDTO correoTemplateNotificacionDTO = new CorreoTemplateNotificacionDTO();
			correoTemplateNotificacionDTO.setKey(Parametros.NOTIFICACION_KEY);
			correoTemplateNotificacionDTO.setValue(idSolicitudBpm);
			CorreoTemplateDTO correoTemplateDTO = new CorreoTemplateDTO();
			correoTemplateDTO.setFileName("L_" + idSolicitudBpm + ".pdf");

			ParametrosWeb param = (ParametrosWeb) UtilidadesWeb.getManagedBean("Parametros");
			@SuppressWarnings("rawtypes")
			HashMap parametros = new HashMap();
			parametros.put("NOAUTORIZACION", numeroAprobacion);// BIEN
			System.out.println(" NOAUTORIZACION "+numeroAprobacion);
//			parametros.put("NOAUTORIZACION", "434358");
			String usurRegistrar = "";
			if (strUsuarioBpm != null) {
				usurRegistrar = strUsuarioBpm;
			} else {
				if (mBUsuario == null) {
			...
			parametros.put("OPERADOR", usurRegistrar);

			Map<String, String> archivo = GenerarReportes.generarArchivoAvalistaGeneralBase64(parametros, correoTemplateDTO.getFileName());
			correoTemplateDTO.setFile(archivo.get(Parametros.ARCHIVO));

			DatosLiquidacion datosLiquidacion = dNConsultasFena.consultaDatosLiquidacion(idSolicitudBpm + "");

			if (datosLiquidacion != null) {

				correoTemplateDTO.setTo(datosLiquidacion.getCorreo());
			}

			correoTemplateDTO.setTemplateId(Parametros.NOTIFICACION_TEMPLATE_LIQUIDACION);

			correoTemplateNotificacionDTO.setCorreo(correoTemplateDTO);
			String respuestaEnvio = enviarCorreoElectronico(correoTemplateNotificacionDTO);

			LogSendLiquidacion registro = new LogSendLiquidacion();
			if (datosLiquidacion != null) {
				registro.setTipoDocumento(new BigDecimal(datosLiquidacion.getTipoDoc()));
				registro.setCorreo(datosLiquidacion.getCorreo());

			}

			registro.setNumeroDocumento(new BigDecimal(voAprobacion.getCedulaGirador()));
			registro.setNombreCliente(voAprobacion.getNombreGirador());
			registro.setFechaRegistro(new Date());
			registro.setSolicitud(idSolicitudBpm);
			registro.setRutaArchivo(Parametros.RUTA_LIQUIDACIONES);
			registro.setNombreArchivo(correoTemplateDTO.getFileName());
			registro.setEstadoEnvio(respuestaEnvio);

			dNConsultasFena.registrarLogSendLiquidacion(registro);

		} catch (Exception e) {
			System.out.println("Error logLiquidacion " + idSolicitudBpm + " " + e.getMessage());
		}

	}
	
	
	public String enviarCorreoElectronico(CorreoTemplateNotificacionDTO correoTemplateNotificacionDTO)
			throws Exception {
		...
		String json = "{\r\n" + "    \"key\": \"" + correoTemplateNotificacionDTO.getKey() + "\",\r\n"
				+ "    \"value\": \"" + correoTemplateNotificacionDTO.getValue() + "\",\r\n" + "    \"correo\": {\r\n"
				+ "        \"to\": \"" + correoTemplateNotificacionDTO.getCorreo().getTo() + "\",\r\n"
				+ "        \"templateId\": \"" + correoTemplateNotificacionDTO.getCorreo().getTemplateId() + "\",\r\n"
				+ "        \"fileName\": \"" + correoTemplateNotificacionDTO.getCorreo().getFileName() + "\",\r\n"
				+ "        \"file\": \"" + correoTemplateNotificacionDTO.getCorreo().getFile() + "\"\r\n" + "    }\r\n"
				+ "}";
		RequestBody body = RequestBody.create(mediaType, json);

	
		Request request = new Request.Builder().url(Parametros.NOTIFICACION_URL_CORREO_TEMPLATE).method("POST", body)
				.addHeader("Authorization", "Bearer " + getTokenNotificaciones())
		...
		return respuestaPagareDto.getMessage();
	...
	public String getTokenNotificaciones() {
		...
		try {
			...
			byte[] message = (Parametros.NOTIFICACION_USUARIO_SERVICIO_AUTH + ":"
					+ Parametros.NOTIFICACION_CONTRASENA_SERVICIO_AUTH).getBytes("UTF-8");
			String encoded = DatatypeConverter.printBase64Binary(message);
			;
			OkHttpClient client = new OkHttpClient().newBuilder().connectTimeout(60, TimeUnit.SECONDS)
					.writeTimeout(60, TimeUnit.SECONDS).readTimeout(60, TimeUnit.SECONDS).build();
			MediaType mediaType = MediaType.parse("application/x-www-form-urlencoded");
			RequestBody body = RequestBody.create(mediaType,
					"username=" + Parametros.NOTIFICACION_USUARIO_SERVICIO + "&password="
							+ Parametros.NOTIFICACION_CONTRASENA_SERVICIO + "&grant_type="
							+ Parametros.NOTIFICACION_GRANT_TYPE + "");
			Request request = new Request.Builder().url(Parametros.NOTIFICACION_URL_SERVICIO_AUTH).method("POST", body)
					.addHeader("Content-Type", "application/x-www-form-urlencoded")
					.addHeader("Authorization", "Basic " + encoded).build();
			Response response = client.newCall(request).execute();
			...
			mensajeRespuestaPagare = "Ocurrio un al generar el token revise el servicio fenalco_integ_notificaciones";
			return "Ocurrio un al generar el token revise el servicio fenalco_integ_notificaciones";
		}
	}
...
//						
						//para desarrollo
//						tempTpagareDetalleLiquidacion.setFechaVencimiento("01-01-1990");
...
//			}
	...
	public String getMensajeRespuestaPagare() {
		return mensajeRespuestaPagare;
	}




	public void setMensajeRespuestaPagare(String mensajeRespuestaPagare) {
		this.mensajeRespuestaPagare = mensajeRespuestaPagare;
	}
