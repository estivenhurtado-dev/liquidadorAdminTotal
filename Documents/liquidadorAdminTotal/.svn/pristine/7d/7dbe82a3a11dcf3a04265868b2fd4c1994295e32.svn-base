...
function validarCaracteres(str) {
	var validos = " 1234567890abcdefghijklmnï¿½opqrstuvwxyzABCDEFGHIJKLMNï¿½OPQRSTUVWXYZ&'.()-_#%\/";
...
function validarEntero(obj, valor) {
	...
	if (isNaN(valor)) {
		...
		// En caso contrario (Si era un nï¿½mero) devuelvo el valor
		// obj.value=valor;
	}
}
...
function validarSoloLetras(str) {
	var validos = " abcdefghijklmnï¿½opqrstuvwxyzABCDEFGHIJKLMNï¿½OPQRSTUVWXYZ";
	for ( var i = 0; i < str.length; i++) {
		if (validos.indexOf(str.charAt(i)) == -1)
			return false;
	}
...
function validarClave(str) {
	var validos = "1234567890abcdefghijklmnï¿½opqrstuvwxyzABCDEFGHIJKLMNï¿½OPQRSTUVWXYZï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ ";
	for ( var i = 0; i < str.length; i++) {
		if (validos.indexOf(str.charAt(i)) == -1)
			return true;
	}
...
/* valida que el aï¿½o sea mayor al siglo pasado */
function y2k(number) {
...
// Mï¿½todo que permite Quitar los espacios en blanco a los lados en las formas
...
// Mï¿½todo que permite saber en un combo se selecciono una opcion si es -1 no
// se selecciono
// Hecho Por Diego Gomez diegomez@jvsoftware.com
...
/*
 * Metodo para validar si una cadena nï¿½merica tipo flotante es positiva
 * Elaborado por Jorge Hernï¿½n Vï¿½lez Modificada por Alejandro Alvarez, para
...
function ValidaForma() {
	...
	transformToUpperCase();// transforma los campos a
							// mayï¿½sculas,jmduran@jvsoftware.com
			...
			if ((val = val.value) != "") {
				...
				if (test.indexOf('isEmail') != -1) {
					...
					if (bandera == false) {
						errors += '- '
								+ nm
								+ ' debe contener una direcciï¿½n de email vï¿½lida.\n';
					}
					...
					if (bandera == false) {
						errors += '- '
								+ nm
								+ ' debe ser una fecha vï¿½lida entre el aï¿½o 1700 y 2300.\n';
					}
															...
															// formato es moneda
				{
					...
					if (bandera == false) {
						errors += '- ' + nm
								+ ' debe ser un formato de moneda vï¿½lido.\n';
					}
				}
				...
				else if (test.indexOf('isLetter') != -1)// JD
				{
					...
					if (swcheck == 0) {
						errors += '- ' + nm
								+ ' requiere seleccionar una opciï¿½n.\n';
					}
				} else if (test.indexOf('isCombo') != -1) {
					if (val == "-1")
						errors += '- ' + nm + ' no se seleccionï¿½.\n';
					...
					if (test.indexOf('isEspecial') != -1) {
						if (!validarCaracteres(val))
							errors += '- '
									+ nm
									+ ' contiene caracteres con tildes o caracteres no vï¿½lidos .\n';
					}
					...
					if (test.indexOf('isNotNum') != -1) {
						...
						if (!isNaN(num))
							errors += '- ' + nm
									+ ' NO debe contener solo nï¿½meros.\n';
					}

				} else if (test.indexOf('isPassword') != -1) {
					if (!validarClave(val))
						errors += '- ' + nm
								+ ' contiene caracteres no vï¿½lidos.\n';
																	...
																	// datos.
				{
					...
					if (isNumeric(val)) {
						...
						errors += '- ' + nm
								+ ' debe ser un nï¿½mero con un mï¿½ximo de '
					...
					if (test.indexOf('isMayor') != -1) {
						...
						if (num <= min)
							errors += '- ' + nm
									+ ' debe contener nï¿½meros mayores a '
									+ min + '\n';
					}
					if (test.indexOf('inRange') != -1) {
						...
						if (num < min || max < num)
							errors += '- ' + nm
									+ ' debe contener solo nï¿½meros entre '
									+ min + ' y ' + max + '.\n';
					}
				}
...
function isDate(val) {
	...
	// inicializarse esta variablï¿½e a travï¿½s del metodo setFormat
	// Modificado por Alejandro Alvarez para validar que las fechas validas
	// esten unicamente entre el aï¿½o 1900 y 2200.
...
// Mï¿½todo que permite deshabilitar o habilitar todos campos de tipo texto de
// una forma
...
// Funciï¿½n que transforma los datos en Mayï¿½sculas a aquellos campos que
...
// el usuario
if (parent == null)
	document.title = "Fenalco Valle ::: Sistema FC Solicitues";// si es una
																// pagina sola
else if (parent.parent != null)
	parent.parent.document.title = "Fenalco Valle ::: Sistema FC Solicitues";// si
																				// es
																				// una
																				// pï¿½gina
																				// dentro
	...
	parent.document.title = "Fenalco Valle ::: Sistema FC Solicitues";// si es
																		// una
																		// pï¿½gina
																		// dentro
																		// de 1
...
// Realizado Por luis alberto Bolaï¿½os
...
function formateafecha(obj) {
	...
	if ((long >= 5) && (segundoslap == false)) {
		...
		// Validacion de Dias por mes sin tomar en cuenta aï¿½o
		if (num == 2) {
			diasMes = 29;
		} else {
...
function formateafechaForm1(obj) {
	...
	if (long >= 10) {
		...
		// Aï¿½o no viciesto y es febrero y el dia es mayor a 28
 ...
 * id de componente Autor: Luis Alberto Bolaï¿½os Email: labs.sistemas@gmail.com
 */
 ...
 * Autor: Luis Alberto Bolaï¿½os Email: labs.sistemas@gmail.com
 */
...
function conteoCaracteres(event) {
	...
	if (str.length > 1998) {
		if ((unicode > 48) || (unicode < 90)) {
			...
			conteoCaracteres.innerHTML = "Ha llegado al lÃ­mite de caracteres permitidos";
			boton.style.display = "none";
		}
	} else {
		numcar = 1998 - str.length;
		conteoCaracteres.innerHTML = "Caracteres mÃ¡ximos " + numcar;
		boton.style.display = "";
	}
}
